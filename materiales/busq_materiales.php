...
$(document).ready(function(){
	...
	$("#id_nuevo").click(function(evento){
		...
		$('#id_materiales').data("idmateriales",0);
		$('#id_materiales').data("materiales",'');
		$('#id_materiales').load("materiales/div_datos.php");

	});

	$( "#text_nombre").autocomplete({
		source: "materiales/buscmateriales.php",
		minLength: 2,
		autoFocus: true,
		select: function(event, ui) {
		  cargar_materiales_lista_todos(ui.item.id);
		},
...
<form role="form" name="f_busq_materiales" id="id_busq_materiales" class="form-horizontal">
